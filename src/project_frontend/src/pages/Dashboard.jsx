...
//   // Fetch dashboard data from the backend canister
//   const fetchData = async () => {
//     if(!callFunction) { console.log("Actor not ready", callFunction)};
//     if (!callFunction) return;
//     try {
//       const data = await callFunction.get_user_data(); // Call Rust backend
...
//         recentProperties: (data.user_registered_properties || []).map(id => ({
//           id,
//           title: `Property ${id}`,
//           location: "Unknown",
//           shares: 0,
...
//   // Poll every 3 seconds when actor is ready and user is authenticated
//   useEffect(() => {
...
//   // Show dashboard even if dashboardData is null at first
//   return (
...
//       <QuickActions />
//       <MyProperties data={dashboardData?.recentProperties ?? []} />
//     </div>
//   );
// };
...
//               <p className="text-sm text-gray-600">{property.shares} shares owned</p>
//             </div>
...
const Dashboard = () => {
  const { isAuth, login, callFunction } = useAuth();
  const [dashboardData, setDashboardData] = useState(null);
  const [properties, setProperties] = useState([]); // 👈 for My Properties
  const [error, setError] = useState(null);
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      const mappedProps = backendProps.map(p => ({
        ...
        shares: p.financial_details?.total_shares ?? 0,
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <QuickActions />
      <MyProperties data={properties} /> {/* 👈 now using fetched props */}
    </div>
  );
};
